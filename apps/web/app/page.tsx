    ...
    <AppLayout>
      ...
      {stats && (
        <section className="max-w-4xl mx-auto px-4 -mt-8 relative z-10">
          <div className="grid grid-cols-3 gap-4">
            <StatCard icon={<BookOpen size={22} />} value={sitesCount} label="收录碑刻" color="vermilion" />
            <StatCard icon={<MapPin size={22} />} value={provincesCount} label="覆盖省份" color="ink" />
            <StatCard icon={<Image size={22} />} value={imagesCount} label="精选图片" color="gold" />
          </div>
        </section>
      )}
      ...
      <section className="max-w-4xl mx-auto px-4 py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <QuickEntry icon={<Clock size={24} />} label="朝代浏览" onClick={() => router.push('/browse?view=timeline')} />
          <QuickEntry icon={<MapPin size={24} />} label="地区浏览" onClick={() => router.push('/regions')} />
          <QuickEntry icon={<ScrollText size={24} />} label="书体分类" onClick={() => router.push('/browse')} />
          <QuickEntry icon={<Compass size={24} />} label="时间轴" onClick={() => router.push('/browse?view=timeline')} />
        </div>
      </section>
      ...
      <section className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="card-stone p-8">
          <h3 className="font-serif text-xl text-ink-600 font-semibold mb-3">参与共建</h3>
...
function StatCard({ icon, value, label, color }: { icon: React.ReactNode; value: number; label: string; color: string }) {
  ...
  return (
    <div className="card-stone p-4 md:p-6 text-center">
      <div className={`w-10 h-10 mx-auto mb-2 rounded-lg flex items-center justify-center ${colorMap[color] || colorMap.ink}`}>
        {icon}
      </div>
      <div className="font-serif text-2xl md:text-3xl font-bold text-ink-600">{value}</div>
      <div className="text-xs text-ink-400 mt-1">{label}</div>
    </div>
  );
}

function QuickEntry({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      <span className="text-sm font-medium text-ink-500 group-hover:text-ink-600">{label}</span>
    </button>
  );
}
