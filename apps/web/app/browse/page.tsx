...
function BrowseContent() {
  ...
  useEffect(() => {
    mockRepository.getPublishedSites({
    ...
    <AppLayout>
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-6">
          <button onClick={() => setView('grid')} className={`px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5 cursor-pointer ${view === 'grid' ? 'bg-vermilion-500 text-white' : 'bg-white text-ink-500 border border-rice-300'}`}>
            <Grid3X3 size={16} /> 网格
          </button>
          <button onClick={() => { setView('timeline'); updateParam('view', 'timeline'); }} className={`px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5 cursor-pointer ${view === 'timeline' ? 'bg-vermilion-500 text-white' : 'bg-white text-ink-500 border border-rice-300'}`}>
            <Clock size={16} /> 时间轴
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="lg:w-60 flex-shrink-0 space-y-6">
            <FilterGroup title="朝代">
              <FilterTag label="全部" active={!activeDynasty} onClick={() => updateParam('dynasty', null)} />
              {dynastyOptions.slice(0, 10).map((d) => (
                <FilterTag key={d} label={d} active={activeDynasty === d} onClick={() => updateParam('dynasty', activeDynasty === d ? null : d)} />
              ))}
            </FilterGroup>
            <FilterGroup title="类型">
              <FilterTag label="全部" active={!activeCategory} onClick={() => updateParam('category', null)} />
              {categoryOptions.map((c) => (
                <FilterTag key={c} label={c} active={activeCategory === c} onClick={() => updateParam('category', activeCategory === c ? null : c)} />
              ))}
            </FilterGroup>
            <FilterGroup title="书体">
              <FilterTag label="全部" active={!activeScript} onClick={() => updateParam('script', null)} />
              {scriptOptions.map((s) => (
                <FilterTag key={s} label={s} active={activeScript === s} onClick={() => updateParam('script', activeScript === s ? null : s)} />
              ))}
            </FilterGroup>
          </aside>
          ...
          <main className="flex-1">
            {sites.length === 0 ? (
              <EmptyState title="暂无符合条件的碑刻" description="请尝试调整筛选条件" />
            ) : view === 'grid' ? (
...
export default function BrowsePage() {
  return (
    <Suspense fallback={<div className="max-w-6xl mx-auto px-4 py-8"><div className="animate-pulse h-64 bg-rice-200 rounded" /></div>}>
      <BrowseContent />
    </Suspense>
  );
}
  ...
  return (
    <div className="space-y-8">
      {Array.from(grouped.entries()).map(([dynasty, items]) => (
        <div key={dynasty} className="relative pl-8 border-l-2 border-vermilion-200">
          ...
          <h3 className="font-serif text-xl text-ink-600 font-semibold mb-3">
            {dynasty}
            <span className="text-sm text-ink-400 font-normal ml-2">{items.length} 处</span>
          </h3>
