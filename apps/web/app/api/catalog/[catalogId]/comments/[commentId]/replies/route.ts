...
import {
  createServerCatalogCommentReply,
  fetchServerCatalogCommentReplyPage,
...
interface ReplyRouteContext {
  params: Promise<{ catalogId: string; commentId: string }>;
}
...
export const GET = async (
  ...
  const { catalogId, commentId } = await context.params;
  try {
    const result = await fetchServerCatalogCommentReplyPage(
      catalogId,
      commentId,
      query,
    );
...
export const POST = async (
  ...
  const { catalogId, commentId } = await context.params;
  try {
    const result = await createServerCatalogCommentReply(
      catalogId,
      commentId,
      token,
      body,
