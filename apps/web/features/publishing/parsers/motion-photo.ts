import { MediaParseError, asciiEquals, fourCc, readHead, u32be } from "./bytes";
import {
  ISOBMFF_LIMITS,
  parseHeifMeta,
  readHeifItem,
...
import { JPEG_LIMITS, listJpegSegments } from "./tiff-exif";

import type { ByteReader } from "./bytes";

/**
 * Google Motion Photo 1.0 containers (a still with an embedded video): XMP
 * `Container:Directory` read with a bounded tokenizer (no DTDs, no entity
 * expansion) and validated against the file layout, mirroring the Backend.
 */

/** Matched by namespace URI, never by prefix. */
  ...
  for (const segment of listJpegSegments(head)) {
    ...
    if (asciiEquals(head, segment.dataStart, MAIN_XMP_HEADER)) {
      ...
      const duplicate = entry.chunks.some(
        ([chunkStart, stop]) => chunkStart === offset && stop === end,
      );
      if (duplicate) {
      ...
      if (
        entry.chunks.some(
          ([chunkStart, stop]) => offset < stop && chunkStart < end,
        )
      )
        throw new MediaParseError("structure_invalid");
...
const tokenizeXml = (text: string): XmlEvent[] => {
  if (text.length > MOTION_PHOTO_LIMITS.maxXmpBytes)
    throw new MediaParseError("limit_exceeded");
  const events: XmlEvent[] = [];
  const push = (event: XmlEvent) => {
...
export interface MotionPhotoXmp {
  readonly items: readonly MotionPhotoDirectoryItem[];
  /** Presentation time of the still within the video in microseconds, if stated (≥ 0). */
  readonly presentationTimestampUs: number | null;
}

const MIME_PATTERN = /^[a-z]+\/[a-z0-9.+-]{1,64}$/u;
const SEMANTIC_PATTERN = /^[A-Za-z]{1,32}$/u;

const decimal = (value: string | undefined): number | null => {
  if (value === undefined) return null;
  const trimmed = value.trim();
  if (
    !new RegExp(`^\\d{1,${MOTION_PHOTO_LIMITS.maxDecimalDigits}}$`, "u").test(
      trimmed,
    )
  )
    throw new MediaParseError("structure_invalid");
...
/**
 * Reads the `Container:Directory` sequence (and the camera presentation
 * timestamp) from one XMP packet, or null when the packet has no directory.
 */
export function readMotionPhotoXmp(xmp: string): MotionPhotoXmp | null {
  ...
  const isElement = (event: XmlEvent, uri: string, local: string) => {
    const resolved = resolve(event.name);
    return resolved?.uri === uri && resolved.local === local;
  };
  let presentationTimestampUs: number | null = null;
  for (const event of events)
    for (const [name, value] of event.attributes) {
      const resolved = resolve(name);
      if (
        resolved?.uri === MOTION_PHOTO_NAMESPACES.camera &&
        resolved.local === "MotionPhotoPresentationTimestampUs" &&
        /^\d{1,15}$/u.test(value.trim())
      )
        presentationTimestampUs = Number(value.trim());
    }
  ...
  return { items, presentationTimestampUs };
}
...
/**
 * Validates a Motion Photo directory against the file: one Primary (first)
 * and one MotionPhoto (last) item, lengths and padding that sum to the file
 * size, an `ftyp` at the computed video offset (with an `mpvd` header for
 * HEIF primaries) and top-level video boxes that tile to the end and include
 * `moov`.
 */
...
export interface MotionPhotoContainer extends MotionPhotoLayout {
  /** The still's presentation time in the motion (ms), when stated and bounded. */
  readonly stillTimeMs: number | null;
}

/**
 * The Motion Photo layout of a JPEG or HEIF still, or null when the still has
 * no container directory. A directory that contradicts the file rejects.
 */
export async function readMotionPhotoContainer(
  reader: ByteReader,
  primaryType: MotionPhotoPrimaryType,
): Promise<MotionPhotoContainer | null> {
  const packets =
    primaryType === "image/jpeg"
      ? readJpegXmpPackets(await readHead(reader, JPEG_LIMITS.maxHeaderBytes))
      : await readHeifXmpPackets(reader);
  for (const packet of packets) {
    const xmp = readMotionPhotoXmp(packet);
    if (xmp === null) continue;
    const layout = await locateMotionPhotoVideo(reader, primaryType, xmp.items);
    const us = xmp.presentationTimestampUs;
    const stillTimeMs =
      us !== null && us / 1000 <= 60_000 ? Math.round(us / 1000) : null;
    return { ...layout, stillTimeMs };
  }
  return null;
}
