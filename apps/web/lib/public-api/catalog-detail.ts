...
const buildCatalogDetailUrl = (
  baseUrl: URL,
  catalogId: string,
): { catalogId: string; url: URL } | undefined => {
  const parsedId = catalogIdSchema.safeParse(catalogId);
  if (!parsedId.success) return undefined;

  return {
    catalogId: parsedId.data,
    url: new URL(`v1/catalog/${encodeURIComponent(parsedId.data)}`, baseUrl),
  };
};
...
export const fetchCatalogDetail = async (
  ...
  const request = buildCatalogDetailUrl(context.baseUrl, catalogId);
  if (request === undefined) return { state: "not-found" };
  ...
  try {
    ...
    if (!parsedDetail.success || parsedDetail.data.id !== request.catalogId) {
