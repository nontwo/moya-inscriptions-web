...
export default function TimelineBrowser({ onSelectDynasty }: TimelineBrowserProps) {
  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-2 mb-2">
        <button onClick={() => scroll('left')} className="p-1 hover:bg-rice-200 rounded cursor-pointer">
        ...
        >
          {data.map((item) => {
            const leftPercent = ((item.yearStart - minYear) / (maxYear - minYear)) * 100;
            const widthPercent = ((item.yearEnd - item.yearStart) / (maxYear - minYear)) * 100;
            ...
            return (
              ...
              >
                ...
                >
                  <span className="text-xs font-medium whitespace-nowrap">{item.dynasty}</span>
                </div>

                {/* 数量徽章 */}
                {hasSites && (
                  <div className={`absolute -top-2 -right-2 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    isSelected ? 'bg-white text-vermilion-500' : 'bg-vermilion-500 text-white'
                  } shadow-md`}>
                    {item.siteCount}
                  </div>
                )}
        ...
        <button onClick={() => scroll('right')} className="p-1 hover:bg-rice-200 rounded cursor-pointer">
      ...
      <div className="h-0.5 bg-rice-300 rounded-full relative mx-6">
        <div className="absolute inset-y-0 bg-vermilion-400 rounded-full" style={{
