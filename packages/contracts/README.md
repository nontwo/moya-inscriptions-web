...
`CatalogId`、三值 `CatalogKind`、`CatalogSummary`、`CatalogDetail` 和
`CatalogPage` 是 T04.1 Phase 1 canonical Public Contract。当前 `ArchiveItem*`
继续服务 T04.0-R `/v1/items` compatibility contract；它不是新的领域模型，
`ArchiveItemId` 在TypeScript中是 `CatalogId` 的兼容名称。
...
- T04.1 Phase
  1 只保留基础list/detail需要的Catalog契约；搜索、分类、图片、Site、Feed和内部生命周期由后续负责任务引入。
