...
export class MockRepository implements ArchiveRepository {
  async getPublishedSites(
    ...
    if (query?.dynasty)
      sites = sites.filter((s) => s.dynasty === query.dynasty);
    if (query?.category)
      sites = sites.filter((s) => s.category === query.category);
    if (query?.scriptType)
      sites = sites.filter((s) => s.scriptType === query.scriptType);
    if (query?.province)
      sites = sites.filter((s) => s.region.province === query.province);
  ...
  async getRelatedSites(siteId: string, limit = 4): Promise<SiteSummary[]> {
    ...
    return mockSites
      ...
      .filter(
        (s) =>
          s.dynasty === site.dynasty ||
          s.region.province === site.region.province,
      )
      .slice(0, limit)
      .map(toSummary);
  }
  ...
  async searchSites(query: SearchQuery): Promise<SearchResult> {
    ...
    if (query.dynasty)
      results = results.filter((s) => s.dynasty === query.dynasty);
    if (query.category)
      results = results.filter((s) => s.category === query.category);
    if (query.scriptType)
      results = results.filter((s) => s.scriptType === query.scriptType);
  ...
  async getDynastyTimeline(): Promise<
    {
      dynasty: Dynasty;
      yearStart: number;
      yearEnd: number;
  ...
  > {
    ...
    return (Object.entries(DYNASTY_ORDER) as [Dynasty, number][]).map(
      ([name]) => ({
        dynasty: name,
