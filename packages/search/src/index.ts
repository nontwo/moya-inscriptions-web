...
export function buildSearchIndex(sites: SiteSummary[]): void {
  siteList = sites.map((site) => ({
    ...
    searchText: [
      ...
      .join(" ")
      .toLowerCase(),
    pinyinFull: pinyin(site.title, {
      toneType: "none",
      type: "string",
    }).replace(/\s/g, ""),
    pinyinInitial: pinyin(site.title, {
      pattern: "first",
      toneType: "none",
      type: "string",
    }).replace(/\s/g, ""),
  ...
  for (const s of sites) {
    ...
    for (const w of words) {
      if (
        w.length > 1 &&
        w.length < 10 &&
        levenshtein(keyword, w.toLowerCase()) <= 2
      ) {
        candidates.add(w);
      }
    }
...
export function search(
  ...
  if (!keyword.trim()) {
    ...
    if (results.length < 3 && !isPinyinLike(searchTerm)) {
      const pinyinResults = fuseIndex!.search(
        pinyin(searchTerm, {
          pattern: "first",
          toneType: "none",
          type: "string",
  ...
  // 筛选
  if (options?.dynasty)
    results = results.filter((s) => s.dynasty === options.dynasty);
  if (options?.category)
    results = results.filter((s) => s.category === options.category);
  if (options?.scriptType)
    results = results.filter((s) => s.scriptType === options.scriptType);
  if (options?.province)
    results = results.filter((s) => s.region.province === options.province);

  const suggestions = keyword
    ? getSuggestions(keyword.toLowerCase(), siteList)
    : [];
