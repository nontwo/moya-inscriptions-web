...
describe("MockRepository", () => {
  ...
  describe("getPublishedSites()", () => {
    it("1.1 无参数应返回全部已发布站点", async () => {
      ...
      expect(sites.every((s) => s.publicationStatus === "published")).toBe(
        true,
      );
    });
    ...
    it("1.7 组合筛选 dynasty + province", async () => {
      const result = await repo.getPublishedSites({
        dynasty: D.TANG,
        province: "湖南省",
      });
      expect(
        result.every(
          (s) => s.dynasty === D.TANG && s.region.province === "湖南省",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("getRelatedSites()", () => {
    it("5.1 有效 siteId 返回相关站点", async () => {
      ...
      for (const r of related) {
        expect(
          r.dynasty === site!.dynasty ||
            r.region.province === site!.region.province,
        ).toBe(true);
      }
    });
  ...
  describe("searchSites()", () => {
    ...
    it('6.2 拼音首字母"dtzxs"应匹配', async () => {
      const result = await repo.searchSites({
        ...BASE_QUERY,
        keyword: "dtzxs",
      });
      expect(result.results.some((r) => r.title === "大唐中兴颂")).toBe(true);
...
describe("export.ts", () => {
  ...
  describe("downloadCSV()", () => {
    it("创建 Blob 并触发下载", () => {
      ...
      const blobArg = (createObjectURL as any).mock.calls[0]?.[0] as
        Blob | undefined;
    ...
    it("CSV Blob 内容含 BOM", () => {
      ...
      const blobArg2 = (createObjectURL as any).mock.calls[0]?.[0] as
        Blob | undefined;
