...
/** 快捷创建 context 工厂 */
function makeContext(overrides: Partial<LogEntry['context']> = {}): LogEntry['context'] {
  return {
    route: '/',
    searchQuery: '',
    userAgent: 'vitest',
...
/** 快捷创建 log 输入工厂 */
function makeInput(overrides: Partial<Omit<LogEntry, 'id' | 'timestamp'>> = {}) {
  return {
...
describe('LogCollector', () => {
  ...
  it('getEntries(level) 应按级别筛选', () => {
    ...
    expect(infos.every((e: { level: string }) => e.level === 'INFO')).toBe(true);
  ...
  it('push() 应保留 stack 字段', () => {
    const entry = collector.push(makeInput({
      level: 'ERROR',
      message: 'stack test',
