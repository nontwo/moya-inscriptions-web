...
let seq = 0;
function uid(): string { return `t${Date.now()}-${seq++}`; }

function pushEntry(msg: string, overrides: Partial<Parameters<typeof logCollector.push>[0]> = {}): LogEntry {
  return logCollector.push({
    level: 'INFO', module: 'Test', function: 'fn', message: msg,
    context: { route: '/', userAgent: 'vitest', screenSize: '1920x1080', browserInfo: 'Chrome/120' },
    ...overrides,
  });
}
...
describe('LogStorage', () => {
  ...
  it('cleanOldLogs() 删除超过 30 天的日志', async () => {
    ...
    await persistLog({
      id: oldId, timestamp: now - 31 * 86400000,
      level: 'INFO', module: 'X', function: 'f', message: 'old-log',
      context: { route: '/', userAgent: 'v', screenSize: '1x1', browserInfo: 'v' },
    });
    await persistLog({
      id: newId, timestamp: now - 1 * 86400000,
      level: 'INFO', module: 'X', function: 'f', message: 'new-log',
      context: { route: '/', userAgent: 'v', screenSize: '1x1', browserInfo: 'v' },
    });
  ...
  it('persistLog() 保留 stack 和 context.extra', async () => {
    ...
    const entry: LogEntry = {
      id, timestamp: Date.now(), level: 'ERROR',
      module: 'X', function: 'f', message: 'stack-test',
      stack: 'Error: boom\n  at test.ts:42',
      context: { route: '/', userAgent: 'v', screenSize: '1x1', browserInfo: 'v', extra: { uid: 99 } },
    };
    await persistLog(entry);
...
describe('ErrorListener', () => {
  ...
  it('window error 触发 GlobalError push', () => {
    ...
    window.dispatchEvent(new ErrorEvent('error', {
      message: 'runtime', error: new Error('runtime'),
  ...
  it('上下文 route 反映当前 location', () => {
    Object.defineProperty(window, 'location', {
      value: { pathname: '/detail/tang', search: '?q=颜', hostname: 'localhost' },
      writable: true, configurable: true,
    });
