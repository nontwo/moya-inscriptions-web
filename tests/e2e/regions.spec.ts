...
test.describe('地区浏览', () => {
  ...
  test('点击省份卡片进入省份详情', async ({ page }) => {
    ...
    // 面包屑出现
    await expect(page.getByRole('button', { name: /全部省份/ })).toBeVisible({ timeout: 5000 });
  ...
  test('面包屑 — "全部省份" 返回列表', async ({ page }) => {
    ...
    // 等面包屑出现
    await page.getByRole('button', { name: /全部省份/ }).waitFor({ timeout: 5000 });
    await page.getByRole('button', { name: /全部省份/ }).click();

    // 应返回省份列表
    await expect(page.locator('.card-stone').first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test('从 Header 导航进入地区页', async ({ page }) => {
    ...
    await page.locator('header').getByRole('link', { name: '地区浏览' }).click();
