...
test.describe('LogButton — 浮动日志按钮', () => {
  ...
  test('LogButton 默认无错误徽章', async ({ page }) => {
    ...
    // 如果页面本身没有触发错误，徽章不应存在
    await expect(badge).not.toBeVisible({ timeout: 1000 }).catch(() => {
...
test.describe('响应式 — 桌面端 (>=768px)', () => {
  test('1920×1080 — Header 桌面导航可见', async ({ page }) => {
    ...
    // 汉堡菜单应隐藏
    await expect(page.locator('button[aria-label="打开菜单"]')).not.toBeVisible();
  });
...
test.describe('响应式 — 全断点综合流程', () => {
  ...
  for (const bp of breakpoints) {
    test(`${bp.name} (${bp.width}×${bp.height}) — 首页正常加载`, async ({ page }) => {
