...
test.describe('分类浏览', () => {
  ...
  test('切换到时间轴视图', async ({ page }) => {
    ...
    // 应显示时间轴视图（含朝代分组）
    await expect(page.locator('.space-y-8, [class*="timeline"]').first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test('筛选 — 朝代标签可点击', async ({ page }) => {
    ...
    if (await tangTag.isVisible()) {
      await tangTag.click();
      // 应过滤结果，卡片仍应存在
      await expect(page.locator('a[href^="/sites/"]').first()).toBeVisible({ timeout: 5000 });
    }
  });
