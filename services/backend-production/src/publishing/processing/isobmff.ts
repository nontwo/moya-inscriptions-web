import { fourCc, i32be, u16be, u32be, u64be } from "./byte-reader.js";
...
/**
 * Parses the box header at `offset` in `bytes`; `bytes[0]` sits at absolute
...
/** Follows a child-type path, e.g. `["trak", "mdia", "hdlr"]`, depth-bounded. */
export function findBoxPath(
  bytes: Uint8Array,
  container: BoxHeader,
  path: readonly string[],
  base = 0,
): BoxHeader | null {
  if (path.length > ISOBMFF_LIMITS.maxDepth) {
    throw new MediaParseError("limit_exceeded");
  }
  let current = container;
  for (const type of path) {
    const next = listBoxes(bytes, current.contentStart, current.end, base).find(
      (box) => box.type === type,
    );
    if (!next) return null;
    current = next;
 ...
 * boxes must tile the file exactly; a size-0 box is only valid when last.
 */
...
/** Reads a whole top-level box of `type` (bounded) with absolute offsets. */
export async function readTopLevelBox(
  reader: ByteReader,
  type: string,
  maxBytes: number,
): Promise<{ box: BoxHeader; bytes: Uint8Array } | null> {
  const box = (await readTopLevelBoxes(reader)).find((b) => b.type === type);
  if (!box) return null;
...
const fullBoxBody = (bytes: Uint8Array, box: BoxHeader, base = 0) => {
  if (box.contentStart + 4 > box.end) throw new MediaParseError("truncated");
  return {
    version: bytes[box.contentStart - base]!,
...
export interface HeifMeta {
  ...
  /** Absolute byte range of `idat` payload within the meta box bytes. */
...
/** Parses a complete HEIF `meta` box (absolute offsets start at 0 = box). */
 ...
 * by `maxBytes`. `metaOffset` is the absolute file offset of the meta box.
 */
  ...
  const describing = exifItems.find((item) =>
    meta.references.some(
      (ref) =>
        ...
        ref.to.includes(meta.primaryItemId),
    ),
  );
  return (describing ?? exifItems[0]!).id;
}

type RightAngle = 0 | 90 | 180 | 270;

/** Clockwise display rotation of a 3x3 QuickTime matrix; null if not a pure right-angle rotation. */
export function matrixRotation(
  bytes: Uint8Array,
  offset: number,
): RightAngle | null {
  const a = i32be(bytes, offset);
  const b = i32be(bytes, offset + 4);
  const c = i32be(bytes, offset + 12);
  const d = i32be(bytes, offset + 16);
  if (b === 0 && c === 0 && a === d && a !== 0) return a > 0 ? 0 : 180;
  if (a === 0 && d === 0 && b === -c && b !== 0) return b > 0 ? 90 : 270;
  return null;
}

/**
 * Clockwise rotation of the first video track composed with the movie
 * matrix, or null when absent or not a pure right-angle rotation.
 */
export function readVideoTrackRotation(
  ...
  if (!mvhd) throw new MediaParseError("structure_invalid");
  const movie = fullBoxBody(moovBytes, mvhd);
  const movieMatrixOffset = movie.bodyStart + (movie.version === 1 ? 44 : 32);
  if (movieMatrixOffset + 36 > mvhd.end) throw new MediaParseError("truncated");
  const movieRotation = matrixRotation(moovBytes, movieMatrixOffset);
  for (const trak of children.filter((box) => box.type === "trak")) {
    const hdlr = findBoxPath(moovBytes, trak, ["mdia", "hdlr"]);
    if (!hdlr || hdlr.contentStart + 12 > hdlr.end) continue;
    if (fourCc(moovBytes, hdlr.contentStart + 8) !== "vide") continue;
    const tkhd = findBoxPath(moovBytes, trak, ["tkhd"]);
    if (!tkhd) throw new MediaParseError("structure_invalid");
    const track = fullBoxBody(moovBytes, tkhd);
    const matrixOffset = track.bodyStart + (track.version === 1 ? 48 : 36);
    if (matrixOffset + 36 > tkhd.end) throw new MediaParseError("truncated");
    const trackRotation = matrixRotation(moovBytes, matrixOffset);
    if (trackRotation === null || movieRotation === null) return null;
    return ((trackRotation + movieRotation) % 360) as RightAngle;
  }
  return null;
}
