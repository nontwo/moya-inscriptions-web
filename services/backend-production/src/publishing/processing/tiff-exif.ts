import { asciiEquals } from "./byte-reader.js";
 ...
 * `bytes[0]`, which is the TIFF header for Exif (or a MakerNote start with an
 * explicit byte order for maker notes).
 */
export class TiffReader {
  ...
  u32(offset: number): number {
    ...
    const [a, b, c, d] = this.bytes.subarray(offset, offset + 4);
  ...
  /**
   * Reads the IFD at `offset`. Each IFD may be visited once per reader, so
   * pointer cycles reject instead of looping.
   */
  ...
  /** First value of a SHORT or LONG entry pointing at another IFD or number. */
...
export interface ExifSummary {
  /** EXIF Orientation 1..8, or null when absent/invalid. */
