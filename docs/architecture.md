...
- T06-A已建立Web server-side Public HTTP data boundary；T06-B.1已实现formal
  ...
  seam；T06-B.2已在不改变transport/loader的前提下接入T02 token、Home
  tabs、本地presentation preferences、branded loading、responsive content
  wall、PublicMedia退化和带active bubble/scroll
  minimize的三端底部浮动主导航。Catalog数据仍由Server
  Component持有；T07–T09仍是后续工作。
