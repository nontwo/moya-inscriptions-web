...
| [0004](0004-catalog-contract-design-freeze.md)            | Accepted   | Catalog Contract 目标设计；implementation pending |

ADR 记录已经接受的架构决定；每份 ADR 必须单独声明 implementation status。
`Accepted` 表示决策已冻结，不自动表示代码已经实现。ADR 0003 继续描述当前 T04.0-R
implemented boundary；ADR 0004 冻结 T04.1
Catalog 目标设计，但尚未开始 implementation。
