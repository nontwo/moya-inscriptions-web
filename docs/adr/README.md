...
受控`catalog-import/v1` CSV validation/dry-run/apply、bounded XLSX
parser、CSV/XLSX convergence与PostgreSQL persistence已经实现；Importer Admin
CRUD、地区审核和正式production数据接入尚未实现。ADR
0002 保留为历史记录，其中的搜索、分类和图片公开边界已由 ADR 0003 取代。ADR
0006 冻结这些能力演进必须遵守的数据权威、identity、provenance、publication 与写入治理边界，不代表提前实现相应 subsystem。
