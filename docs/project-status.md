...
工程、设计与部署方案基线已经建立，业务开发尚未开始。旧 T01 把特定来源数据与公共契约混合，现已撤回并移出应用仓库；新的 T01 将从来源无关的平台档案模型重新建立。

`integration/mvp` 当前集成 T00、T02、T03 和独立手机交互原型；`main`
继续保留稳定的 T00/治理基线。

| 任务     | 状态       | 当前成果                                                  |
| -------- | ---------- | --------------------------------------------------------- |
| T00      | 已完成     | pnpm/Turborepo Monorepo、Web/Admin/API 骨架、CI、协作治理 |
| T01      | 等待重建   | 来源无关的档案领域类型、Public DTO 与生命周期契约         |
...
API 只有纯函数健康检查；来源无关公共契约、数据库 Schema、Repository、搜索、图片管线、登录、地图、互动、上传、生产环境和正式部署都不存在。
...
当前测试覆盖工程 fixture、T02
token/资产/组件/架构和手机原型交互；新的 T01将在独立 PR 中重新建立契约测试。
...
1. T01：建立来源无关的档案领域类型、Public DTO、生命周期和 runtime schema。
2. T04：基于新 T01 建立 Repository、正式 Public API 和架构边界。
3. T05：建立 object key 驱动的图片适配与处理管线，以及经批准的自动化脚本。
