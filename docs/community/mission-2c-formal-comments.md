...
| Navigation                | Back/Forward between Home/Browse/Search, Detail and Viewer, the reading position and `?catalogId=` direct entry behave exactly as before; closing Detail returns to the originating page and scroll. On pc and tablet-landscape the accepted layout folds the reading flow behind 详情 whenever a comment section is composed; a layout change (on tablet, any orientation change) re-folds it, and the remembered position is applied as far as the folded content reaches (inherited from #106, already so on the preview). | Same Detail without a comment section, so nothing folds.                      | Product History model untouched.                                                                                                                 |
