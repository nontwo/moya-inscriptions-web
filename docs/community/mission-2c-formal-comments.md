...
| Ready                     | Hot roots (≤ 3, Backend-ranked) first, then the latest page of 10, `加载更多评论` until the last page; each root shows its first three replies and `查看更多回复（还有 N 条）` pages the rest.           | —                                                                             | No sort control, no like control, no media upload.                                                                                               |
...
| Listing unavailable       | 503 → "评论服务暂时不可用" state (`data-comment-unavailable`), no count, no composer claim of success.                                                                                                   | —                                                                             | Detail itself still renders.                                                                                                                     |
