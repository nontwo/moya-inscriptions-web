...
| Receipt read path for community receipts                                                                          | BUILT            | every agent comment moderation writes an execution receipt bound to its operation, target, action and canonical command, in the transition's own transaction; recovery and cancel read it |
...
- Stall window (closed): an executor that applies part of a chunk and loses its
  lease before the chunk write no longer undercounts. Each comment transition
  commits an execution receipt in its own transaction, bound to the operation,
  target index, target, action and canonical command, so the next holder
  recovers the exact applied result, the tally is truthful and the undo covers
  it. Events written before this change carry no receipt and are not attributed
  to any operation.
