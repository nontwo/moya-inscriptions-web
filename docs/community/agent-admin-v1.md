...
| Receipt read path for community receipts                                                                          | PARTIAL          | built for comment moderation only: each agent comment transition writes an execution receipt bound to its operation, target, action and canonical command inside the transition's own transaction, and recovery and cancel read it. `content_operator_receipts` and `author_command_receipts` still have no read path |
...
- Stall window (open for featured targets): `setFeatured` writes its own
  per-target receipt, but nothing reads it, so a featured target whose chunk
  write was lost is reported `conflict` on resume and `cancelled` in a cancelled
  remainder. Those recommendation rows are real, the tally denies them and the
  undo excludes them. Closing it needs the symmetric read-only lookup on the
  content-operator port; it is not closed here because the recommendation
  surface is outside this revision's scope.
