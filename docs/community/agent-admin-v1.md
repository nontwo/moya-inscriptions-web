...
| Undo                                                     | Finished, non-undo operations with applied targets; `hide`↔`unhide` only (approve/reject → conflict `UNDO_NOT_AVAILABLE`); featured restores the recorded prior row guarded by the post-operation version; a new operation needing its own approval.          |
