...
const quickActionHooks = {
  collect: (contentId) =>
    logQaEvent("quick-action", `[quick-action] collect ${contentId}`),
  like: (contentId) =>
    logQaEvent("quick-action", `[quick-action] like ${contentId}`),
  share: (contentId) =>
    logQaEvent("quick-action", `[quick-action] share ${contentId}`),
};
...
function commitQuickAction(action, gesture) {
  ...
  const hook = quickActionHooks[action];
  if (!label || !hook) {
    closeQuickAction();
    return;
  }
  quickActionStatus.textContent = `已选择${label}`;
  hook(gesture.contentId);
}
