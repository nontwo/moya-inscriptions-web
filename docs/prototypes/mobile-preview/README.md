...
896 与方向变化时保持当前条目与选中图像。Hero 为作品主图加悬浮信息框（身份 / 简介 / 基本资料）；说明、图像和资料来源留在主图之后。

## 样式隔离

- `preview.shared.css`：三端一致的结构、卡片、专题、表单与加载样式。
- `preview.css`：手机触摸分页、横竖屏内容密度及始终位于底部的液态玻璃底栏覆盖。
- `preview.tablet.css`：平板触摸分页、双列专题，以及横竖屏均回到底部浮动胶囊的覆盖。
- `preview.pc.css`：PC 滚动、底部液态玻璃导航、更密首页/书帖卡片与触控板分页覆盖。
