...
平板 768–895px 竖屏使用底部导航，横屏使用 88px 左侧轨；宽屏平板继续使用紧凑左侧轨。PC 从 896px 起使用固定放大侧栏、宽屏卡片密度、强化格言及碑刻主从栏，并在 1024px、1440px 继续放大。

运行时监听窗口缩放和方向变化，只切换样式表及壳层行为，不刷新页面。打开的碑刻详情在跨越 896px 时会在全屏详情和 PC 主从栏之间转换。

## 样式隔离

- `preview.css`：手机基础样式，无 768px 以上布局规则。
...
关联测试：`tests/integration/prototypes/mobile-preview.test.ts`。建议检查 360、390、430、768、834、896、1024、1440、2048px，并覆盖手机、Android
